...
const nodeId =
    process.argv[4] || process.env.HIVE_NODE_ID || require('os').hostname();

if (!hubUrl || !secret) {
    console.error('Usage: hive-worker <hub-url> <secret> [node-id]');
    console.error(
        '  or set HIVE_HUB_URL, HIVE_SECRET, and optionally HIVE_NODE_ID env vars',
    );
    process.exit(1);
}
    ...
    ws.on('message', async (raw) => {
        ...
        if (msg.type === 'rpc') {
            try {
                ...
                ws.send(
                    JSON.stringify({type: 'rpc:response', id: msg.id, result}),
                );
            } catch (err) {
                ws.send(
                    JSON.stringify({
