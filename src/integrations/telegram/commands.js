...
function mdToHtml(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (
            ...
            escapedTrimmed.length < 40 &&
            !escapedTrimmed.endsWith('.') &&
            !escapedTrimmed.endsWith(':') &&
        ...
        if (numList) {
            ...
            out.push(
                `${indent}<b>${idx[2]}.</b> ${applyInlineFormatting(content)}`,
            );
            continue;
        }
...
async function status(config, send, router) {
    ...
    for (const s of sessions) {
        ...
        if (s.pr) {
            ...
            lines.push(
                `         PR#${pad(s.pr.prNum, 6)} +${rpad(s.pr.prAdds, 4)} -${rpad(s.pr.prDels, 4)}${ci}${rv}`,
            );
        }
    }
...
async function idle(config, send, router) {
    const sessions = (await fleet.getFleetStatus(config, router)).filter(
        (s) => s.state === 'idle',
    );
    if (sessions.length === 0) {
...
async function working(config, send, router) {
    ...
    for (const s of sessions) {
        ...
        if (s.pr) {
            const ci = s.pr.ciResult ? `  ${ciTag(s.pr.ciResult)}` : '';
            lines.push(
                `    PR#${s.pr.prNum} +${s.pr.prAdds} -${s.pr.prDels}${ci}`,
            );
        }
    }
...
async function session(config, send, router, query) {
    ...
    if (s.pr) {
        lines.push('');
        lines.push(
            `PR #${s.pr.prNum}  +${s.pr.prAdds} -${s.pr.prDels}  (${s.pr.prFiles} files)`,
        );
        if (s.pr.review) lines.push(`Review:  ${reviewTag(s.pr.review)}`);
        if (s.pr.ciBuild)
            lines.push(
                `CI:      ${ciTag(s.pr.ciResult)}  Jenkins #${s.pr.ciBuild}`,
            );
    } else {
        lines.push(`PR:      none`);
    }
...
async function restart(config, send, router, query) {
    ...
    setTimeout(async () => {
        ...
        send(
            `\u267b\ufe0f Restarting Claude in session ${fleet.sessionNum(name)}...`,
            HTML,
        );
        setTimeout(async () => {
