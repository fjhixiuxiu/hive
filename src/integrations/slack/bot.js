...
function createSlackBot(taskQueue, config, router) {
    ...
    if (!appToken || !botToken) {
        console.log(
            'Missing SLACK_APP_TOKEN or SLACK_BOT_TOKEN — Slack bot disabled',
        );
        return null;
    }
    ...
    async function handleMessage(event, say) {
        ...
        if (!text) {
            await say({
                text: 'What do you need? Try: `@hive <task>` or `@hive status`',
                thread_ts: event.ts,
            });
            return;
        }
            ...
            if (
                ...
                try {
                    ...
                    if (found) {
                        ...
                        await relay.tell(config, node, found.name, text, {
