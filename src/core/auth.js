        ...
        try {
            ...
            if (!user || !user.login) {
                ...
                return res
                    .status(502)
                    .send(
                        `GitHub API error: ${reason}. Please try again later.`,
                    );
            }
            ...
            const isAdmin =
                adminUser &&
                adminUser.toLowerCase() === user.login.toLowerCase();
            const isPreAdded =
                taskQueue && taskQueue.getUser(user.login.toLowerCase());
            if (!isAdmin && !isPreAdded) {
                return res
                    .status(403)
                    .send('Access denied — ask your Hive admin to add you.');
            }
...
function authenticateWebSocket(msg, request) {
    ...
    if (isOAuthEnabled()) {
        ...
        if (user) {
            return {
                authenticated: true,
                user: {login: user.sub, name: user.name, avatar: user.avatar},
            };
        }
