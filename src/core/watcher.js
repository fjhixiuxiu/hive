...
const QUESTION_IGNORE = [/\?\s+for shortcuts/, /\?\s+for help/, /Try "/];
...
class Watcher extends EventEmitter {
    ...
    async start() {
        ...
        this.approvalInterval = setInterval(() => {
            this._checkApprovals().catch((err) =>
                console.error('Approval check error:', err.message),
            );
        }, 10000);
    }
    ...
    async _poll() {
        ...
        for (const s of sessions) {
            ...
            if (
                currState === 'idle' &&
                !this.notifiedIdle.has(s.num) &&
                this.seenWorking.has(s.num)
            ) {
                if (this.detectedWaiting.has(s.num)) {
                    ...
                    if (count >= 5) {
                        ...
                        try {
                            ...
                            if (node) {
                                preview = await fleet.peekSession(
                                    this.config,
                                    node,
                                    s.name,
                                );
                                ...
                                ansiSnapshot =
                                    (await node.exec(
                                        `tmux capture-pane -e -p -S -500 -t "${paneTarget}" 2>/dev/null`,
                                    )) || '';
    ...
    async _checkApprovals() {
        ...
        for (const s of sessions) {
            ...
            if (!isWaiting) {
                ...
                for (const line of lines) {
                    if (
                        OPTION_PATTERN.test(line) ||
                        NUMBERED_OPTION_PATTERN.test(line)
                    )
                        optionCount++;
