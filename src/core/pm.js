...
class ProjectManager extends EventEmitter {
    ...
    rescan(id) {
        ...
        if (pm.enabled) {
            this._poll(id).catch((err) =>
                console.error(`Rescan error for ${pm.name}:`, err.message),
            );
        }
    ...
    loadState(pmsData) {
        ...
        for (const data of pmsData) {
            ...
            const pm = {
                ...
                autoThreshold:
                    data.autoThreshold != null ? data.autoThreshold : 3,
    ...
    _createManualTask(id) {
        ...
        let taskText = text;
        if (pm.taskFormat) {
            taskText = pm.taskFormat
                .replace('{key}', key)
                .replace('{summary}', text);
        }
        const fullText = pm.instructions
            ? `${taskText}\n\nInstructions: ${pm.instructions}`
            : taskText;
        const task = this.taskQueue.createTask(
            fullText,
            mode,
            pm.targetSession || null,
            pm.designation,
            {source: `pm:${pm.name}`},
        );
        ...
        this.taskQueue.pushFeed(
            'task',
            null,
            `PM "${pm.name}" created manual task`,
        );
        this._save();
        this.emit('pm:changed');
    }
    ...
    async _poll(id) {
        ...
        try {
            ...
            switch (pm.source.type) {
                case 'jira':
                    issues = await this._fetchJira(pm.source);
                    break;
                case 'github-issues':
                    issues = await this._fetchGithubIssues(pm.source);
                    break;
                case 'github-prs':
                    issues = await this._fetchGithubPrs(pm.source);
                    break;
                case 'jenkins':
                    issues = await this._fetchJenkins(pm.source);
                    break;
                case 'zoho':
                    issues = await this._fetchZoho(pm.source);
                    break;
                case 'github-re-reviews':
                    issues = await this._fetchReReviews(pm.source, pm.id);
                    break;
                default:
                    throw new Error(
                        `Unsupported source type: ${pm.source.type}`,
                    );
            }
            pm.lastPoll = Date.now();
            pm.lastError = null;
            ...
            for (const issue of issues) {
                ...
                if (issue._alreadyQueued) {
                    ...
                    this._commentOnPR(issue._repo, issue._prNumber, body).catch(
                        (err) => {
                            console.error(
                                `Failed to comment on PR #${issue._prNumber}:`,
                                err.message,
                            );
                        },
                    );
                ...
                const mode = pm.targetSession
                    ? 'manual'
                    : this._evaluateComplexity(issue, pm.autoThreshold);
                let text;
                if (pm.taskFormat) {
                    text = pm.taskFormat
                ...
                if (
                    pm.source.type === 'github-prs' ||
                    pm.source.type === 'github-re-reviews'
                ) {
                    ...
                    if (prInfo) {
                        ...
                        this._commentOnPR(
                            ...
                            console.error(
                                `Failed to comment on PR #${prInfo.prNumber}:`,
                                err.message,
                            );
                        });
                    }
    ...
    async _fetchJira(source) {
        ...
        if (!baseUrl || !email || !apiToken) {
            throw new Error(
                'JIRA credentials not configured (JIRA_BASE_URL, JIRA_EMAIL, JIRA_API_TOKEN)',
            );
        }
    ...
    async _fetchGithubPrs(source) {
        if (!source.repo) throw new Error('GitHub repo not configured');
        const allowedBases = source.base ? [source.base] : ['main', 'master'];
    ...
    async _fetchJenkins(source) {
        ...
        if (!baseUrl || !user || !token)
            throw new Error(
                'Jenkins credentials not configured (JENKINS_URL, JENKINS_USER, JENKINS_API_TOKEN)',
            );
    ...
    async _fetchZoho(source) {
        ...
        if (!orgId || !token)
            throw new Error(
                'Zoho Desk credentials not configured (ZOHO_DESK_ORG_ID, ZOHO_DESK_API_TOKEN)',
            );

        let urlStr;
        if (source.query) {
            const params = new URLSearchParams({
                searchStr: source.query,
                limit: '50',
            });
            if (source.department)
                params.set('departmentId', source.department);
    ...
    async _fetchReReviews(source, pmId) {
        if (!source.repo) throw new Error('GitHub repo not configured');
        if (!source.reviewer)
            throw new Error('Reviewer username not configured');

        const triggers = (
            source.triggerPhrases ||
            'ready for review,ptal,please review,addressed'
        )
            .split(',')
            .map((t) => t.trim().toLowerCase())
            .filter(Boolean);
        if (triggers.length === 0)
            throw new Error('No trigger phrases configured');
        ...
        for (const pr of prs) {
            ...
            if (
                lastPoll &&
                new Date(pr.updated_at).getTime() < lastPoll - 120000
            )
                continue;
            ...
            let comments;
            try {
                comments = await this._httpRequest(commentsUrl, headers);
            } catch (e) {
                continue;
            }
            ...
            for (const comment of comments) {
                ...
                if (login.endsWith('[bot]') || comment.user?.type === 'Bot')
                    continue;
                ...
                (res) => {
                    ...
                    res.on('end', () => {
                        if (res.statusCode >= 200 && res.statusCode < 300) {
                            ...
                            reject(
                                new Error(
                                    `HTTP ${res.statusCode}: ${resBody.slice(0, 200)}`,
                                ),
                            );
                        }
                    });
