...
class TaskQueue extends EventEmitter {
    constructor(config, watcher, router) {
            ...
            try {
                ...
                for (const [num, taskId] of this.activeTaskBySession) {
                    ...
                    if (s && s.state === 'idle') {
                        ...
                        console.log(
                            `[reconcile] S:${num} is idle with dispatched task: "${(task?.text || '').slice(0, 60)}"`,
                        );
                    }
                }
                if (staleCount > 0) {
                    console.log(
                        `[reconcile] ${staleCount} dispatched task(s) on idle sessions — watcher will handle transitions`,
                    );
                }
            } catch (err) {
        ...
        for (const [sessionNum, taskId] of this.activeTaskBySession) {
            ...
            if (task) {
                const ts =
                    task.lastActivityAt || task.dispatchedAt || task.createdAt;
                if (ts && this.watcher)
                    this.watcher.sessionActivity.set(sessionNum, ts);
            }
        }
    ...
    attachTask(text, sessionNum, meta) {
        ...
        this.pushFeed(
            'task',
            sessionNum,
            `Task attached to session ${sessionNum}: "${text}"`,
        );
        this._saveState();
        return task;
    }
    ...
    async _dispatchTask(task, sessionNum) {
        ...
        if (this.watcher)
            this.watcher.sessionActivity.set(sessionNum, Date.now());
        this.activeTaskBySession.set(sessionNum, task.id);
        ...
        const sendTask = async () => {
            if (task.mode === 'auto') {
                const clearResult = await relay.tell(
                    this.config,
                    node,
                    sessionName,
                    '/clear',
            ...
            const desigName =
                task.designation || this.designations.get(sessionNum);
            const desigDef = desigName
                ? this.designationDefs.get(desigName)
                : null;
            if (
                desigDef &&
                desigDef.agentFiles &&
                desigDef.agentFiles.length > 0
            ) {
                const parts = [];
            ...
            return relay.tell(this.config, node, sessionName, fullMessage, {
                vimMode: this.vimMode,
            });
        };
    ...
    async _tryAutoDispatch() {
        ...
        try {
            ...
            const manualTargeted = Array.from(this.tasks.values()).filter(
                (t) =>
                    t.status === 'queued' &&
                    t.mode === 'manual' &&
                    t.targetSession,
            ...
            for (const task of manualTargeted) {
                const session = sessions.find(
                    (s) => s.num === task.targetSession,
                );
                if (
                    session &&
                    session.state === 'idle' &&
                    !this.dispatchLock.has(session.num) &&
                    !this.activeTaskBySession.has(session.num)
                ) {
            ...
            idleAuto.sort(
                (a, b) =>
                    (this.lastDispatchedAt.get(a.num) || 0) -
                    (this.lastDispatchedAt.get(b.num) || 0),
            );
            ...
            for (const session of idleAuto) {
                const task = queuedTasks.find((t) => {
                    ...
                    if (t.designation) {
                        return (
                            this.designations.get(session.num) === t.designation
                        );
                    }
    ...
    setSpawnSlotRange(min, max) {
        ...
        this._saveState();
        this.emit('spawnSlotRange:changed', {
            min: this.spawnSlotMin,
            max: this.spawnSlotMax,
        });
    }
    ...
    ensureUser(login, name, avatar) {
        ...
        const isAdmin =
            isFirstUser ||
            (adminUser && adminUser.toLowerCase() === login.toLowerCase());
        user = {
            login,
            name: name || login,
            avatar: avatar || '',
            permissions: isAdmin
                ? [...TaskQueue.ALL_PERMISSIONS]
                : ['view', 'comment'],
            firstSeen: Date.now(),
        };
        ...
        this._saveState();
        console.log(
            `[auth] User "${login}" registered (${isAdmin ? 'admin' : 'viewer'})`,
        );
        return user;
    }
    ...
    setUserPermissions(login, permissions) {
        ...
        user.permissions = permissions.filter((p) =>
            TaskQueue.ALL_PERMISSIONS.includes(p),
        );
        this.emit('users:changed', this.getUsersList());
        this._saveState();
        return user;
    ...
    deleteComment(taskId, commentId, requestingLogin) {
        ...
        if (
            comment.author !== requestingLogin &&
            !this.hasPermission(requestingLogin, 'admin')
        ) {
            return false;
        }
    ...
    async spawnSession({num, baseDir, name, gitUrl} = {}) {
        ...
        if (num === undefined || num === null) {
            ...
            if (!slots.length)
                throw new Error(
                    `No available slots (${this.spawnSlotMin}-${this.spawnSlotMax} all occupied)`,
                );
            num = slots[0];
        }
        if (num < this.spawnSlotMin || num > this.spawnSlotMax)
            throw new Error(
                `Spawn slots must be ${this.spawnSlotMin}-${this.spawnSlotMax}`,
            );
        ...
        try {
            execSync(
                `/bin/zsh -lc 'tmuxinator start -p ${agentYml} N=${num} ROOT="${repoDir}"'`,
    ...
    pushFeed(type, session, detail, extra) {
        ...
        while (this.feed.length && this.feed[0].timestamp < threeDaysAgo)
            this.feed.shift();
        this._debounceSave();
    ...
    _loadState() {
        try {
            ...
            if (data.spawnSlotMin !== undefined)
                this.spawnSlotMin = data.spawnSlotMin;
            if (data.spawnSlotMax !== undefined)
                this.spawnSlotMax = data.spawnSlotMax;
            // Restore tasks
            if (Array.isArray(data.tasks)) {
                for (const t of data.tasks) {
                    ...
                    if (Number(t.id) >= nextTaskId)
                        nextTaskId = Number(t.id) + 1;
                }
            }
            // Restore feed
            ...
            console.log(
                `Loaded state: ${this.autoSessions.size} auto-sessions, ${this.designations.size} designations, ${this.designationDefs.size} defs, ${this.agentRoots.length} agent roots, ${this.spawnedAgents.size} spawned agents, ${this.users.size} users`,
            );
            if (this.tasks.size)
                console.log(`Restored ${this.tasks.size} tasks`);
            if (this.feed.length)
                console.log(`Restored ${this.feed.length} feed entries`);
        } catch {
    ...
    _wireWatcher() {
        this.watcher.on('session:idle', (data) => {
            ...
            this.pushFeed(
                'state',
                data.num,
                `Session ${data.num} went idle`,
                extra,
            );
            this._handleSessionIdle(
                data.num,
                data.ansiSnapshot || data.preview,
                data.paneCols,
            );
        ...
        this.watcher.on('session:working', (data) => {
            this.pushFeed(
                'state',
                data.num,
                `Session ${data.num} started working`,
            );
